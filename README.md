...
测试包含离线文件加载、Logo 与 GitHub 入口、默认音效、键盘与触屏操作、碰撞、跳跃、低头、护盾、失败重试、三关通关与刷新后存档恢复，以及手机竖屏和横屏布局；测试通过推进真实物理逻辑验证关卡路径，触屏检查由浏览器模拟完成
...
│   ├── assets/pelican-logo.png # 鹈鹕骑车透明 Logo，构建时内嵌
│   ├── world.js            # 渲染、场景与关卡配置
│   ├── rider.js            # 鹈鹕、单车模型与动画
│   ├── course.js           # 障碍、拾取物与粒子
