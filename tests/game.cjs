...
(async()=>{
 ...
 try{
  ...
  check('Chinese defeat dialog is fully translated',await page.locator('#result').textContent().then(text=>[zh.lostTag,zh.lostTitle,zh.lostDesc,zh.retry,zh.home,zh.rideStat,zh.fishStat,zh.scoreStat].every(value=>text.includes(value))));
  ...
  for(let level=0;level<6;level++){
   ...
   check('Chinese stage '+(level+1)+' victory dialog is localized',await page.locator('#result').textContent().then(text=>[level===5?zh.allTitle:zh.wonTitle,level===5?zh.allTag:zh.wonTag,level===5?zh.again:zh.next,zh.home].every(value=>text.includes(value))));
  ...
  await page.reload();await waitForGame(page);check('completed stages persist after reload',await page.evaluate(()=>save.unlocked===6&&save.stars.length===6&&save.best.length===6&&save.stars.every(n=>n===3)&&save.best.every(n=>n>0)));
  check('unlocked route buttons persist',await page.locator('.route:disabled').count(),0);
  check('no JavaScript page errors',errors,[]);
  const mobile=await browser.newContext({viewport:{width:390,height:844},isMobile:true,hasTouch:true,locale:'zh-CN',colorScheme:'light'});await mobile.setOffline(true);
  ...
  for(const viewport of [{width:320,height:568},{width:390,height:844},{width:568,height:320},{width:844,height:390}]){
   ...
   check(size+' header fits without overlap',await phone.evaluate(()=>{const brand=document.querySelector('.brand').getBoundingClientRect(),actions=document.querySelector('.top-actions').getBoundingClientRect(),sub=document.querySelector('.brand-sub').getBoundingClientRect();return brand.left>=0&&brand.top>=0&&brand.right<=innerWidth&&brand.bottom<=innerHeight&&actions.left>=0&&actions.top>=0&&actions.right<=innerWidth&&actions.bottom<=innerHeight&&(brand.right<=actions.left||actions.right<=brand.left||brand.bottom<=actions.top||actions.bottom<=brand.top)&&sub.width>0&&sub.height>0&&[...document.querySelectorAll('.top-actions button,.top-actions a,.top-actions select')].filter(b=>b.getClientRects().length&&!b.hidden).every(b=>{const r=b.getBoundingClientRect();return r.width>=44&&r.height>=44&&Boolean(b.getAttribute('aria-label')||b.textContent.trim())})}));
