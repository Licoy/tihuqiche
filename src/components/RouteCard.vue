...
<template>
  <button class="route" :class="[level.id, { active: selected === index, locked }]" :disabled="locked" :aria-pressed="selected === index" :aria-label="label" :title="label" :data-level="index" @click="selectLevel(index)">
    ...
    <span class="route-copy"><span class="route-name">{{ levelName(index) }}</span><span class="route-state"><FontAwesomeIcon v-if="locked" :icon="faLock" />{{ locked ? copy.locked : save.records[selectedMode].stars[index] ? '★'.repeat(save.records[selectedMode].stars[index]) : copy.ready }}</span></span>
  </button>
</template>
