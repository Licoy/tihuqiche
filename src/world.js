...
// One factory owns the existing scene setup and GPU resources through teardown.
...
const ambientLight = new T.HemisphereLight(0xfff7dc,0x508c87,2.3); scene.add(ambientLight);
...
let darkMode=false;
function setTheme(index, dark=darkMode) {
 darkMode=dark;
...
function setLanguage(locale) {
 ...
 bannerCtx.fillText(locale==='en'?'FINISH':'终点',256,55);banner.material.map.needsUpdate=true;
}
...
function dispose(){
 ...
 scene.traverse(o=>{if(o.isInstancedMesh)o.dispose();if(o.geometry)geometries.add(o.geometry);if(o.material)materials.add(o.material)});
 Object.values(geos).forEach(g=>geometries.add(g));mats.forEach(m=>materials.add(m));
