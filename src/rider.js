...
import { createRiderEffects } from './rider-effects.js';
import { addBirdDetails } from './rider-details.js';
...
export function createRider(world, initialConfig=defaultRiderConfig()) {
 ...
 const effects=createRiderEffects(world,rider),{shieldBubble}=effects;
 function applyConfig(nextConfig){
  ...
  model=replacement;config=next;rider.add(model.root);effects.attach(model.bike,model.exhaust,config.vehicle);
 }
 function animateRider(frame){
  if(disposed)throw new Error('Rider has been disposed');
  model.animate(frame);effects.animate(frame);rider.position.y=frame.jumpY;
 }
 function dispose(){
  if(disposed)return;
  disposed=true;effects.dispose();rider.removeFromParent();rider.clear();model=null;
 }
 applyConfig(config);
...
const {bike,wheels,feet,drive,pose,animate:animateVehicle,exhaust,setSteering}=createVehicle(world,root,config);
...
const animateFace=addBirdDetails(world,{bird,head,palette});
...
let pedalAngle=0;
function animate({dt,time,moving,jumpY,duck,speed,boosting,steering=0}){
 ...
 if(animateVehicle)animateVehicle({dt,time,moving,boosting});
 setSteering(steering);animateFace(time);
 ...
 feet.forEach((foot,i)=>{
  ...
  if(drive?.pedals){
   ...
   y=pose?.footY??(config.vehicle==='motorcycle'?.83:.77);z=pose?.footZ??(config.vehicle==='motorcycle'?.13:-.32);
   knee=[side*.39,pose?.kneeY??1.2,pose?.kneeZ??-.33];
  }
  foot.position.set(side*.3,y,z);
  const hipY=duck?1.48:pose?.hipY??(standing?1.88:1.73);
 ...
 bird.position.y=(pose?.birdY??(standing?1.98:1.83))+(moving?Math.sin(pedalAngle*2)*.025:0);
...
return {root,bike,exhaust,animate};
}
