...
function temple(parent,x,z){
 ...
 for(let i=0;i<11;i++) {
  ...
  const themes = Array.from({length:6},()=>{const g=new T.Group();segment.add(g);return g});
 ...
 for(let i=0;i<8;i++) box('#7e9296',[-45+i*13,3+i%3*3,-170],[7,10+i%3*6,8],skyline);
 rod('#9daaad',[4,-2,-160],[4,35,-160],{radius:.55,parent:skyline});
 for(const y of [12,26]) orb('#b07883',[4,y,-160],[3.4,3.4,3.4],skyline);
 return { decor, skyline };
}
