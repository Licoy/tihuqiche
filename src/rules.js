...
export function createPlayer(id = 0) {
 return {id,status:'running',distance:0,speed:0,lane:1,x:0,jumpY:0,vy:0,duck:0,hp:3,
  fishCollected:0,fishBalance:0,bonusScore:0,shield:false,invincible:0,boostRemaining:0,
...
export function applyAction(player, type) {
 ...
 else if (type === 'boost' && player.fishBalance >= 10 && player.boostRemaining === 0) {
  player.fishBalance -= 10; player.boostRemaining = 2.5;
 } else return false;
 return true;
}
...
export function advancePlayers(game, dt) {
 ...
 for (const p of running) {
  ...
  p.speed = base * Math.min(1.75, 1 + (p.boostRemaining > 0 ? .5 : 0) + p.catchupBonus);
  p.distance = game.gameMode === 'endless' ? p.distance + p.speed * dt : Math.min(level.length, p.distance + p.speed * dt);
  p.x += ((p.lane - 1) * 3.4 - p.x) * (1 - Math.exp(-14 * dt));
  if (p.vy !== 0 || p.jumpY > 0) {
   p.jumpY += p.vy * dt - 11 * dt * dt; p.vy -= 22 * dt;
   if (p.jumpY <= 0) { p.jumpY = 0; p.vy = 0; }
  }
  for (const field of ['duck','invincible','boostRemaining','magnetRemaining','doubleRemaining']) p[field] = Math.max(0, p[field] - dt);
 }
 syncOverview(game);
}
...
export function settlePlayers(game) {
 for (const p of game.players) if (p.status === 'running') {
  if (p.hp <= 0) { p.status = 'downed'; p.speed = 0; }
  else if (game.gameMode !== 'endless' && p.distance >= LEVELS[game.level].length) { p.status = 'finished'; p.speed = 0; }
 }
 syncOverview(game);
